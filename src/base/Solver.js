...
class Solver {
  ...
  #solve() {
    ...
    for (let i = 0; i < zeroSum.length; i++) {
      ...
      const wallCount =
        this.map.rows.reduce((total, row) => total + row, 0) + this.map.cols.reduce((total, col) => total + col, 0);
    ...
    //?Display
    console.log(zeroSum);
    console.log(this.targetSolutions);
    console.log(this.solutionFound);
    for (let k = 0; k < 8; k++) {
      this.solutionFound.push(this.targetSolutions[k].combinations[zeroSum[0][k]].data);
    }
    const adn = { genes: [] };
    for (let i = 0; i < this.solutionFound.length; i++) {
      const row = this.solutionFound[i];
  ...
  #findLowestIndex() {
    ...
    for (let i = 1; i < this.map.cols.length; i++) {
      ...
      if (Math.abs(4 - col) > this.map.cols[lowestIndex]) {
        lowestIndex = i;
      }
    }
    return lowestIndex;
